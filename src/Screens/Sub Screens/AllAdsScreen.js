...
import { View, Text, StyleSheet, ScrollView, TextInput, Image, Touchable, TouchableOpacity, FlatList, ActivityIndicator, RefreshControl } from 'react-native';
...
export default function AllAdsScreen({ navigation, route }) {
    ...
    const catnaam = route.params;
    console.log(data);

    React.useEffect(() => {
        fetchCurrentCountryData();   setTimeout(() => {
    ...
    const fetchCurrentCountryData = () => {
        fetch('https://www.jeemjam.com/api/get-ads-fetching')
            ...
            .then((json) => setData(json.getads.filter(u => u.country == catnaam?.country?.name)))
    ...
    return (
        <View style={styles.container}>
            ...
            <View style={styles.list}>
                {isLoading ? 
                    <View style={{ marginHorizontal: '40%', marginVertical: '50%', alignItems: 'center' }}>
                    ...
                    <FlatList
                        ...
                        refreshControl={            <RefreshControl refreshing={isLoading} onRefresh={fetchCurrentCountryData} />
