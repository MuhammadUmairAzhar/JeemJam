...
export default function MyAds({navigation, route}) {
  ...
  console.log(
    'AHAAAAAAAAAAAAAAAAllll ADSs Dataaa ================>>>>>>',
    authName,
  );
  ...
  React.useEffect(() => {
    ...
    fetch(`https://www.jeemjam.com/api/show-ads/user/${authName}`)
  ...
  const delPost = id => {
    fetch(`https://www.jeemjam.com/api/deletePost/${id}`, {method: 'DELETE'})
      .then(res => {
          ...
          renderItem={({item, index}) => (
            <View key={index} style={styles.listelem}>
              ...
              <View
                ...
                <View>
                  <TouchableOpacity
                    onPress={() => navigation.navigate('EditAd', item)}>
                    <FontAwesome
                      name="edit"
                      size={30}
                      color={'#0000a5'}
